...
const ChatPage = () => {
  ...
  return (
    <main className="min-h-[calc(100svh-4rem)] bg-[#d9d9d9] p-0 text-[#070b1a] md:p-5">
      <section className="mx-auto flex min-h-[calc(100svh-4rem)] w-full max-w-6xl flex-col overflow-hidden border border-[#eef2f8] bg-[#e5e5e5] shadow-2xl md:min-h-[calc(100svh-6.5rem)] md:rounded-[2rem]">
        <header className="border-b border-[#d8dee8] bg-[#e7e7e7] px-3 py-4 sm:px-5 md:px-9 md:py-7">
          <div className="flex items-center gap-2 sm:gap-4">
            <Button
              ...
              className="h-10 w-10 shrink-0 rounded-full bg-white/70 text-[#172033] shadow-sm hover:bg-white md:hidden"
            >
              <ArrowLeft className="h-5 w-5" />
            </Button>

            <div className="relative h-12 w-12 shrink-0 overflow-visible rounded-[1rem] bg-[#171717] shadow-lg sm:h-16 sm:w-16 sm:rounded-[1.25rem] md:h-[4.7rem] md:w-[4.7rem] md:rounded-[1.4rem]">
              {friend.avatar ? (
                <img
                  src={friend.avatar}
                ...
                <div className="flex h-full w-full items-center justify-center text-xl font-black text-white sm:text-2xl md:text-3xl">
                  {friendInitial}
                </div>
              )}
            ...
            <div className="min-w-0">
              <h1 className="truncate text-xl font-black leading-tight text-[#070b1a] sm:text-2xl md:text-3xl">
                {friend.username}
              </h1>
              <div className="mt-1 flex items-center gap-2 text-xs font-bold text-[#69768a] sm:text-sm md:text-base">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-5 sm:py-7 md:px-9 md:py-9">
              ...
              {messages.map((msg, idx) => {
                ...
                return (
                  <div key={`${msg.timestamp || idx}-${idx}`} className={`flex w-full ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`flex max-w-[92%] flex-col sm:max-w-[88%] md:max-w-[76%] ${mine ? 'items-end' : 'items-start'}`}>
                      <div
                        className={`rounded-[1.25rem] px-4 py-3 shadow-md sm:rounded-[1.55rem] sm:px-5 sm:py-4 md:px-7 md:py-5 ${
                          mine
                            ? 'rounded-br-[0.45rem] bg-[#171717] text-white'
                            : 'rounded-bl-[0.45rem] border border-[#dce3ee] bg-[#fbfbfc] text-[#1d2738]'
                        }`}
                      >
                        <p className="whitespace-pre-wrap break-words text-[0.95rem] font-medium leading-6 sm:text-[1rem] sm:leading-7 md:text-[1.1rem]">
                          {msg.msg}
                        </p>
                      </div>
                      <time className="mt-2 text-xs font-bold text-[#91a0b7] sm:mt-3 sm:text-sm md:text-base">
        ...
        <footer className="border-t border-[#d8dee8] bg-[#f4f4f4] px-3 py-4 sm:px-5 sm:py-5 md:px-9 md:py-7">
          <div className="flex items-end gap-2 sm:gap-3 md:gap-5">
            <Button
              ...
              className="hidden h-12 w-12 shrink-0 rounded-[1rem] bg-white text-[#223047] shadow-sm hover:bg-[#fafafa] min-[420px]:inline-flex sm:h-14 sm:w-14 sm:rounded-[1.25rem] md:h-[4.4rem] md:w-[4.4rem]"
            >
            ...
            <textarea
              ...
              placeholder="Message..."
              rows={1}
              className="max-h-36 min-h-12 flex-1 resize-none rounded-[1rem] border border-[#dce3ee] bg-white px-4 py-3 text-sm font-semibold leading-6 text-[#172033] shadow-sm outline-none transition placeholder:text-[#95a2b8] focus:border-[#bac5d6] focus:ring-2 focus:ring-white sm:min-h-14 sm:rounded-[1.4rem] sm:px-5 sm:py-4 sm:text-base md:min-h-[4.4rem] md:px-7 md:py-5 md:text-lg"
            />
            <Button
              type="button"
              onClick={sendMessage}
              disabled={!canSend}
              className="h-12 shrink-0 rounded-[1rem] bg-[#171717] px-4 text-base font-black text-white shadow-lg hover:bg-[#262626] disabled:bg-[#171717]/40 sm:h-14 sm:rounded-[1.25rem] sm:px-5 md:h-[4.4rem] md:px-7 md:text-lg"
            >
